...
const TIKTOK_SCOPES = [
  // Solo scopes del Login Kit que existen y se usan. `user/info/` lee open_id,
  // display_name y avatar_url (basic) mas username (profile), y los
  // contadores de cuenta salen de stats.
  //
  // NO anadir `user.info.username` ni `user.account.type`: el primero no
  // existe y el segundo no lo usa nadie. TikTok rechazaba el OAuth entero con
  // «scope» por culpa de ellos.
  "user.info.basic",
  "user.info.profile",
  "user.info.stats",
  // Contadores por video: vistas, me gusta, comentarios y compartidos. Los
  // guardados no los expone TikTok en ninguna forma.
  "video.list",
